    ...
    contents.split("\n").enumerate().for_each(|(u_row, line)| {
        ...
        line.chars().enumerate().for_each(|(u_column, char)| {
            ...
            if char.is_ascii_digit() {
                ...
                let part = Part {
                    end_point: point,
                    length: i32::try_from(cur_number.len()).unwrap(),
                    value: cur_number.parse().unwrap(),
                };
        ...
        if !cur_number.is_empty() {
            let end_point = Point {
                row,
                column: i32::try_from(line.len()).unwrap() - 1,
            };
            let part = Part {
                end_point,
                length: i32::try_from(cur_number.len()).unwrap(),
...
fn check_part_number(part_number: &Part, symbol_points: &HashSet<Point>) -> Vec<Point> {
    ...
    for column in column_start..=column_end {
        let above_point = Point {
            row: part_number.end_point.row - 1,
            column,
        };
...
pub fn problem_a(contents: String) -> u32 {
    ...
    let valid_part_numbers: Vec<Part> = part_numbers
        .into_iter()
        .filter(|part_number| !check_part_number(part_number, &symbol_points).is_empty())
        .collect();
    valid_part_numbers
        .iter()
...
pub fn problem_b(contents: String) -> u32 {
    ...
    let part_points: Vec<(Vec<Point>, Part)> = part_numbers
        .into_iter()
        .map(|part_number| (check_part_number(&part_number, &symbol_points), part_number))
        .collect();

    let mut point_to_parts = HashMap::new();
    part_points.iter().for_each(|(points, part)| {
