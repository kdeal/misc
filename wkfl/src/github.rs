...
impl GitHubClient {
    ...
    pub fn new(host: String, token: String) -> Self {
        let (api_base, graphql_endpoint) = if host == "github.com" {
            (
    ...
    fn api_graphql<T: serde::de::DeserializeOwned>(
        ...
        let resp = ureq::post(&self.graphql_endpoint)
            .set("Authorization", &format!("Bearer {}", &self.token))
            .set("User-Agent", "wkfl")
    ...
    /// Get review/diff comments for a PR using GraphQL API
    fn get_review_comments(
        ...
        // Extract comments from the nested GraphQL response
        let data = graphql_resp
            .data
            .ok_or_else(|| anyhow!("GraphQL response missing data field"))?;

        let mut comments = Vec::new();
        for thread in data.repository.pull_request.review_threads.nodes {
            for gql_comment in thread.comments.nodes {
                // Convert GraphQL comment to ReviewComment
                comments.push(ReviewComment {
                    body: gql_comment.body,
                    user: User {
                        login: gql_comment.author.login,
                        user_type: gql_comment.author.typename,
                    },
                    created_at: gql_comment.created_at,
                    path: gql_comment.path,
                    original_line: gql_comment.original_line,
                    original_start_line: gql_comment.original_start_line,
                    diff_hunk: gql_comment.diff_hunk,
                    side: gql_comment.side,
                    start_side: gql_comment.start_side,
                });
            }
        }
